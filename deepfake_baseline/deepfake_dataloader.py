...
class Deepfake_Dataset(Dataset):
    ...
    def __init__(self, root, dataset_name, split_type='train', img_size=299,
                ...
                comp_type='raw', hq=True, infinite_mode=False, attack_test=False):
        ##########################################################################################################################
        self.root = root
        ...
        if self.random_sample_num is None or self.debug or (self.infinite_mode and self.split_type=='train'):
    ...
    def sample_frame_list(self):
        # print(self.dataset_name, len(self.all_real_frame_list), len(self.all_fake_frame_list ))
    ...
    def get_frame_from_image(self, path):
        ...
        if self.quality < 100:
            frame = Image.fromarray(self.compressor(image=np.asarray(frame))['image'])
    ...
    def get_frame_list(self, fake=True):
        ...
        for ii, (video_file) in enumerate(video_list):
            if self.verbose:
                print("\r{} videos {:04}/{:04}".format(frame_label.lower(), ii+1,total_iter), end='')
            ...
            if sample_num is not None:
                if sample_num > video_total_frame:
                    # print(video_file, video_total_frame)
                    continue
